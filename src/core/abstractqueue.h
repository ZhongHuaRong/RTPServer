...
public:
	...
	using pointer				= std::shared_ptr<Packet>;
	using PacketQueue			= std::queue<pointer>;
public:
	AbstractQueue():
...
//		auto flag = _queue_read_condition.wait_for(lk,std::chrono::milliseconds(millisecond));
	 ...
	 * @return 
	 */
	inline virtual pointer get_next() noexcept{
	 ...
	 */
	inline virtual pointer get_latest() noexcept{
		std::lock_guard<std::mutex> lk(_mutex);
		if(_queue->empty())
			return nullptr;
		while(_queue->size() > 1){
	 ...
	 * 新的包
	 */
	inline void push_one(pointer newPacket) noexcept{
